...
describe('TextInput', () => {
  ...
  it('changes color when focused', async () => {
    ...
    expect(wrapper.get(titledInputSelector).classes()).not.toContain('border-violet-500')

    await wrapper.get(textInputSelector).trigger('focus')
    expect(wrapper.get(titledInputSelector).classes()).toContain('border-violet-500')
