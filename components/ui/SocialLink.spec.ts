...
describe('SocialLink', () => {
    ...
    it('renders with set username', () => {
        const wrapper = mount(SocialLink, defaultMountOptions)
        expect(wrapper.get(linkSelector).attributes('href')).toContain(defaultMountOptions.props.username)
    })
    it('renders with set url', () => {
        const wrapper = mount(SocialLink, defaultMountOptions)

        expect(wrapper.get(linkSelector).attributes('href')).toContain(defaultMountOptions.props.url)
    })
    ...
    it('renders with set icon', () => {
        ...
        expect(wrapper.get(iconSelector).attributes('name')).toBe(defaultMountOptions.props.icon)
    })
    it('renders with set width', () => {
        const testWidth = '199px'

        const wrapper = mount(SocialLink, { ...defaultMountOptions, props: { ...defaultMountOptions.props, width: testWidth } })
    ...
    it('renders with set height', () => {
        ...
        const wrapper = mount(SocialLink, { ...defaultMountOptions, props: { ...defaultMountOptions.props, height: testHeight } })
