...
describe('YoutubeLink', () => {
    it('renders with set size and username', () => {
        ...
        expect(wrapper.get(youtubeLinkSelector).attributes('href')).toBe(`https://www.youtube.com/@${testUsername}`)
        expect(wrapper.get(youtubeIconSelector).attributes('width')).toBe(testWidth)
        expect(wrapper.get(youtubeIconSelector).attributes('height')).toBe(testHeight)
    })
})