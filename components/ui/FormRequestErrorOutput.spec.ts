...
describe('FormRequestErrorOutput', async () => {
    ...
    it('renders unexpected error output without "status" prop provided', () => {
        ...
        expect(wrapper.get(errorTitleSelector).text()).toBe('Произошла непредвиденная ошибка');
        expect(wrapper.get(errorDescriptionSelector).text()).toBe('Проверьте ваше интернет соединение');
    })
    it('renders wrong login credentials error output with "status" prop = "401"', () => {
        ...
        expect(wrapper.get(errorTitleSelector).text()).toBe('Неверные данные для входа');
    })
    it('renders confirm email error output with "status" prop = "403"', () => {
        ...
        expect(wrapper.get(errorTitleSelector).text()).toBe('Подтвердите электронную почту');
    })
