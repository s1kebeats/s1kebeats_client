...
describe('MenuLink', () => {
  describe('props', () => {
    it('to - should render with set to param on nuxt-link', () => {
      ...
      expect(wrapper.get(menuLinkSelector).attributes('to')).toBe(
        defaultMountOptions.props.to
      );
    });
    it('icon - should render with set icon', () => {
      const wrapper = shallowMount(MenuLink, defaultMountOptions);
      expect(wrapper.get(menuLinkIconSelector).attributes('name')).toBe(
        defaultMountOptions.props.icon
      );
    });
    it('text - should render with set text', () => {
      const wrapper = shallowMount(MenuLink, defaultMountOptions);
      expect(wrapper.get(menuLinkTextSelector).text()).toBe(
